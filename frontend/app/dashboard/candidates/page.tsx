...
import { 
  User, Check, X, Briefcase, Calendar, MessageSquare, Eye, 
  MapPin, Instagram, Facebook, Globe, Share2, Hash 
} from 'lucide-react';
...
export default function CandidatesPage() {
  ...
  return (
    <div className="animate-fade-in max-w-6xl mx-auto pb-20 relative">
            ...
            <div key={app.id} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
              <div className="flex flex-col md:flex-row gap-6 items-center justify-between">
                ...
                <div className="flex gap-3 w-full md:w-auto mt-4 md:mt-0">
                  {app.status === 'pending' ? (
                    <>
                      <button onClick={() => handleReject(app.id)} className="px-4 py-2 rounded-xl text-red-500 font-bold bg-red-50 hover:bg-red-100 transition-colors">Rechazar</button>
                      <button onClick={() => handleAcceptAndChat(app.id)} className="px-6 py-2 rounded-xl text-white font-bold bg-[var(--color-brand-dark)] hover:bg-green-600 transition-colors shadow-md flex items-center gap-2"><MessageSquare size={18}/> Aceptar y Chatear</button>
                    </>
                  ) : (
                    <button onClick={() => router.push(`/dashboard/chat/${app.id}`)} className="px-4 py-2 rounded-xl text-[var(--color-brand-dark)] font-bold border border-[var(--color-brand-dark)] hover:bg-gray-50 flex items-center gap-2"><MessageSquare size={18}/> Ir al Chat</button>
                  )}
                </div>
              </div>
            </div>
      ...
      {/* --- MODAL "PRO" (DISEÑO IDÉNTICO A MI PERFIL) --- */}
      {selectedProfile && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in overflow-y-auto">
          {/* Contenedor Modal más ancho para el diseño PRO */}
          <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden relative my-8">
            
            {/* Botón Cerrar Flotante */}
            <button 
              onClick={() => setSelectedProfile(null)}
              className="absolute top-4 right-4 bg-white/90 p-2 rounded-full hover:bg-gray-100 z-20 transition-colors shadow-sm text-gray-600"
            >
              <X size={24}/>
            </button>
            ...
            <div className="px-6 pb-8 md:px-10">
               ...
               <div className="flex flex-col md:flex-row items-end -mt-16 mb-6 gap-6">
                  <div className="w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-white shadow-xl bg-white overflow-hidden relative z-10">
                     ...
                     ) : <div className="w-full h-full flex items-center justify-center bg-gray-100 text-gray-400"><User size={64}/></div>}
                  </div>
                  
                  <div className="flex-1 pb-2 text-center md:text-left">
                     <h2 className="text-3xl font-bold text-gray-900 flex items-center justify-center md:justify-start gap-2">
                        {selectedProfile.full_name}
                        {selectedProfile.is_verified && <span className="text-blue-500">✓</span>}
                     </h2>
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  ...
                  {/* Derecha: Redes Sociales (Clickables) */}
                  <div className="space-y-4">
                     <h3 className="font-bold text-gray-800 flex items-center gap-2">
                        <Share2 size={20} className="text-[var(--color-brand-orange)]" /> Conecta
                     </h3>

                     {selectedProfile.instagram_handle ? (
                        <a href={selectedProfile.instagram_url || `https://instagram.com/${selectedProfile.instagram_handle.replace('@','')}`} target="_blank" rel="noopener noreferrer" className="group flex items-center justify-between p-4 bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-md hover:border-pink-200 transition-all">
                           <div className="flex items-center gap-3">
                              ...
                              <div><p className="font-bold text-gray-800">Instagram</p><p className="text-xs text-gray-500">{selectedProfile.instagram_handle}</p></div>
                           </div>
                           <div className="text-gray-300 group-hover:text-pink-500">→</div>
                        </a>
                     ) : <div className="p-3 border border-dashed rounded-xl text-center text-gray-400 text-sm">Sin Instagram</div>}

                     {selectedProfile.tiktok_handle ? (
                        <a href={selectedProfile.tiktok_url || `https://tiktok.com/@${selectedProfile.tiktok_handle.replace('@','')}`} target="_blank" rel="noopener noreferrer" className="group flex items-center justify-between p-4 bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-md hover:border-black transition-all">
                           <div className="flex items-center gap-3">
                              ...
                              <div><p className="font-bold text-gray-800">TikTok</p><p className="text-xs text-gray-500">{selectedProfile.tiktok_handle}</p></div>
                           </div>
                           <div className="text-gray-300 group-hover:text-black">→</div>
                        </a>
                     ) : <div className="p-3 border border-dashed rounded-xl text-center text-gray-400 text-sm">Sin TikTok</div>}

                     {selectedProfile.facebook_handle && (
                        <a href={selectedProfile.facebook_url || '#'} target="_blank" rel="noopener noreferrer" className="group flex items-center justify-between p-4 bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-md hover:border-blue-200 transition-all">
                           <div className="flex items-center gap-3">
                              <div className="w-10 h-10 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center"><Facebook size={20} /></div>
                              <div><p className="font-bold text-gray-800">Facebook</p><p className="text-xs text-gray-500">{selectedProfile.facebook_handle}</p></div>
                           </div>
